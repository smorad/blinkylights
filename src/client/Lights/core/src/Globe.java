   ...
   public void FillColors()
   {
   	...
   	for (int i =0; i < rows; i++ ) 
      {
         for (int j = 0; j < columns; j++ ) 
         {
         	...
         	int at = ((int)Mod(j + layers.get(0).x, layers.get(0).width)  + (i+(int)layers.get(0).y) * columns)*4;

         	int r = layers.get(0).GetPixelAt(((at+0)%count + count) % count);
         	int g = layers.get(0).GetPixelAt(((at+1)%count + count) % count);
         	int b = layers.get(0).GetPixelAt(((at+2)%count + count) % count);
         	int a = layers.get(0).GetPixelAt(((at+3)%count + count) % count);
